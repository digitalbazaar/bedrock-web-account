...
export class AccountService {
  ...
  */
   ...
   * @param {object} options - The options to use.
   * @param {string} [options.baseUrl = /accounts] - The base baseUrl.
   * @param {object} options.meter - Meter to create.
   * @param {object} options.account - The account to create the meter with.
   *
   * @returns {Promise<object>} - Created meter.
   */
  async createMeter({
    baseUrl = this.config.urls.base, account, meter
  }) {
    const url = `${baseUrl}/${account.id}/meters`;
    const response = await httpClient.post(url, {json: {meter}});
    return response.data;
  }
}
